    ...
    const renderItem = ({ item }) => {
        return (
            <View style={props.estado ? estilo.sliderBox : {marginRight: 10}}>
                <Pressable onPress={() => props.navegacao.navigate("Details", {name: item.name, source: item.source, by: item.by, about: item.about, video: item.video ? item.video : null, topicos: item.topicos ? item.topicos : null, infos: item.Infos ? item.Infos : null, id: item.id, video : item.video ? item.video : null})}>
